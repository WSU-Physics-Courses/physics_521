...
def my_lambdify(args, expr):
    # Sort args in decreasing order of derivatives so we replace
    # derivatives before the expressions.
    def order(a):
        a = a[0]
        _s = a.atoms(sympy.Derivative)
        if _s:
            return -max(len(_d.variables) for _d in _s)
        else:
            return 0
    args = list(args)
...
def get_rhs(L, funcs, var, simplify=False):
    """Returns a function that acts as the rhs for the first-order ODE
    resulting from the euler_lagrange equations.

    For now we assume that each function appears
    """
    _x = sympy.symbols('_x')
    ...
    for eq in eqs:
        row = []
        for df in dfuncs:
            _cs = sympy.Poly(eq, df).all_coeffs()
            assert len(_cs) == 2
            row.append(_cs[0])
            eq = _cs[1]
        row.append(-eq)
        A.append(row)
    res = sympy.solvers.solve_linear_system(sympy.Matrix(A), *dfuncs,
                                            simplify=simplify)
    return res
...
def _euler_equations(L, funcs=(), vars=()):
    r"""
